...
class MoodWallpaperQuizView(View):
    ...
    def _generate_questions(self):
        settings = [
            "personality traits (e.g., bravery, kindness)",
            "colors (e.g., favorite colors, colors representing feelings)",
            "objects (e.g., nature, animals, dreams, movies, books, music)",
            "patterns (e.g., geometric shapes, floral designs)",
            "future goals (e.g., dreams, aspirations)",
            "values (e.g., friendship, family, adventure)",
            "emotions (e.g., happiness, sadness, excitement)",
            "activities (e.g., hobbies, sports, creative pursuits)",
        ]
        
        default_questions = [
            "If your current emotional state was a color, what would it be and why?",
            "Which fictional character do you relate to most right now: a peaceful monk, an adventurous explorer, or a creative artist?",
            "Imagine your mind is a landscape - what kind of scenery do you see today?",
            "If you could be anywhere in the world right now, where would you be and what would you be doing?",
            "What animal or object best represents your current mood?"
        ]

        try:
            prompt = (
                "Generate 5 short, very simple and short questions that a everyone would understand. Questions should help people understand their current mood and personality. "
                "Also do not ask questions directly, be creative"
                "Example questions: Over the last two weeks have you been feeling down, depressed, irritable, or hopeless?"
                "Which character from the following list is most similar to you:... and why"
                "Last question should be like: Please write any random object, animal thath you associate with your current mood"
            )
            ...
            return questions if len(questions) == 5 else default_questions
...
class MoodWallpaperView(APIView):
    def post(self, request):
        try:
            ...
            dalle_prompt, wallpaper_title = self._generate_prompts(data)
    ...
    def _generate_prompts(self, data):
        ...
        mood_character = (
            ...
            "If questions ask about specific objects or animals, please look at the answers and include such association objects "
            "(colors, animals, celestial bodies etc.) in your response"
            "Result format should be like: {\"mood_values\": \"energetic, optimistic, creative, seeking growth, socially connected\", "
            "\"object_associations\": \"sea, orcas, etc.\"}"
        )
        ...
        mood_prompt = (
            ...
            "Your result should be in this JSON format: {\"prompt\": \"your detailed image generation prompt here\", "
        ...
        try:
            ...
            return result_dict['prompt'], result_dict['title']
    ...
    def _save_wallpaper(self, user, title, width, height, image_url):
...
class StandardResultsSetPagination(PageNumberPagination):
    page_size = 9  # Changed to 9 items per page
    page_size_query_param = 'page_size'
    max_page_size = 9  # Also set max_page_size to 9 to enforce this limit
